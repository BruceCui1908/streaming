...
public:
    ...
    static ptr create()
    {
        return ptr(new buffer_raw());
    }

    buffer_raw(size_t capacity = 0)
    {
        set_capacity(capacity);
    }

    buffer_raw(const char *data, size_t size = 0)
    {
        assign(data, size);
    }
    ...
    void set_size(size_t size)
    {
        if (size > capacity_)
        {
            throw std::invalid_argument("buffer cannot set size bigger than capacity");
        }
